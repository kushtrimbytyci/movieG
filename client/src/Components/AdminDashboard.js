...
const AdminDashboard = () => {
  ...
  const themoviedb = async () => {
    const res = await axios.get(
      `https://api.themoviedb.org/3/search/movie?api_key=${process.env.REACT_APP_TM}&query=${search}`
    );
    let genre1;
    ...
    setForma({
      ...
      imgsrc: `https://image.tmdb.org/t/p/w300${res.data.results[0].poster_path}`,
      desc: res.data.results[0].overview,
      topimdb: res.data.results[0].vote_average,
      year: year[0],
      quality: "HD",
    });
  };
  ...
  return (
    <div
      ...
      style={{
        paddingTop: "50px",
        backgroundColor: `${profileCon.backgroundColor}`,
      }}
    >
