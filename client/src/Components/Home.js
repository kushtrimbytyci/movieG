...
import { Pagination, Loader, Alert, Toggle, Icon } from "rsuite";
...
const Home = () => {
    ...
    const getMovies = async () => {
      try {
        const data = await axios.get(
          `/api/movies?limit=48&docex=true&sort=-year`,
          { cancelToken: source.token }
        );
        profileCon.setProfile(data.data.data.data);
        for (let i = 0; i <= 20; i++) {
          arr.push({
            link: data.data.data.data[i].link,
            id: data.data.data.data[i]._id,
            title: data.data.data.data[i].title,
            genre: data.data.data.data[i].genre,
            src: data.data.data.data[i].imgsrc,
          });
        }
        setSlide(arr);
        setShow(true);
  ...
  const changePage = async (value, e) => {
    profileCon.clearProfile();
    const data = await axios.get(
      `/api/movies?limit=48&page=${value}&docex=true&sort=${sort}`
    );
    profileCon.setProfile(data.data.data.data);
    setPage(Math.ceil(data.data.data.count / 48));
    setNext(value);
      ...
      >
        ...
        >
          <TextLoop interval={2000} style={{ backfaceVisibility: "hidden" }}>
            <span>
              <img src={signs} alt="signs" width="15px" /> Don't forget to
              bookmark Gomovie.site
            </span>
            <span>
              <img src={multimedia} alt="multimedia" width="15px" /> Best movie
              collection on the internet!!
            </span>
          </TextLoop>
        </div>
        <div
          style={{ maxWidth: "100vw", marginTop: "10px" }}
        ...
        <div
          className="daynight daynight-radial-out"
          onClick={() => toggleChecker(!profileCon.toggle)}
        >
        ...
        <div
          className="cim"
          style={{ display: "flex", top: "30px", marginBottom: "40px" }}
        >
          <button
            onClick={() => {
              profileCon.clearProfile();
              ...
              const getMovies = async () => {
                try {
                  const data = await axios.get(
                    `/api/movies?limit=48&docex=true&sort=-createdAt`
                  );
                  profileCon.setProfile(data.data.data.data);
                  setPage(Math.ceil(data.data.data.count / 48));
                } catch (error) {
