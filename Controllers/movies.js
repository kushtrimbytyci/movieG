...
exports.addMovie = asyncHandler(async (req, res, next) => {
  ...
  movie = await MovieSchema.create({title,imgsrc,link,genre,year,desc,quality,topimdb});
  res.status(200).json({ success: true, data: movie });
});


exports.updateMovie = asyncHandler(async(req,res,next)=>{
  const movie = await MovieSchema.findOneAndUpdate({_id:req.params.title},req.body,{new:true,runValidators:true})
  if(!movie)
  {
...
exports.likeQuery = asyncHandler(async(req,res,next)=>{
  ...
  const movies= await MovieSchema.find({title:{'$regex': `${search}`, '$options': 'i'}}).limit(3) 
  if(movies.length===0)
  {
